...
class ActivitiesManagementOrchestratorApiExceptionHandler {
  ...
  fun handleMissingServletRequestParameterException(e: MissingServletRequestParameterException): ResponseEntity<ErrorResponse> = ResponseEntity
    ...
    .body(
      ErrorResponse(
        status = BAD_REQUEST,
        userMessage = "Missing parameter: ${e.message}",
        developerMessage = e.message,
      ),
    ).also { log.info("Missing request parameter: {}", e.message) }
  ...
  fun handleHttpMessageNotReadableException(e: HttpMessageNotReadableException): ResponseEntity<ErrorResponse> = ResponseEntity
    ...
    .body(
      ErrorResponse(
        status = BAD_REQUEST,
        userMessage = "Invalid request body: ${e.message}",
        developerMessage = e.message,
      ),
    ).also { log.info("Invalid request body: {}", e.message) }
